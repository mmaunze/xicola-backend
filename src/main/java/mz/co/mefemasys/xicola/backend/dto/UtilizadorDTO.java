...
public class UtilizadorDTO {
    ...
    private String getSingleRoleAsString(Set<Role> roles) {
        if (roles == null || roles.isEmpty()) {
            return "";
            // Retorna uma string vazia se não houver roles
        }

        // Obtém a primeira role disponível
        Role firstRole = roles.iterator().next();

        return mapRoleToString(firstRole);
        // Mapeia a role para string
    }
    ...
    private String mapRoleToString(Role role) {
        if (role == null) {
            return "";
            // Retorna uma string vazia se a role for nula
        }
        ...
        switch (role.getName()) {
            ...
            default:
                return "";
            // Retorna uma string vazia se a role não corresponder
        }
    }
}
